...
class a4130 extends Base{

	public function a4130(){
		parent::Base();
	}
	...
	private function _list($all = false){
		...
		$fld = 'H.id,H.generate_date,H.generate_time,H.kdn_member_id,M.company_name,M.charge_name,M.telno,H.note1,H.note2,H.note3';
	...
	private function _listView(){
		...
		while($row = $result->fetch(PDO::FETCH_ASSOC)){
			...
			$res .= "<tr>\n";
       		...
       		$res .= "  <td ".$class." >".$row['generate_date']."<br />".$row['generate_time']."</td>\n";
       		$res .= "  <td ".$class.">".nl2br($row['note1'])."</td>";
			...
			$s = (axts('charge_name',$row)) ? $row['charge_name'] : '';
       		$res .= "  <td ".$class."><a href=\"".$link."\">".$s."</a></td>\n";
       		$res .= "  <td ".$class." valign=\"top\">".$row['telno']."</td>\n";
	...
	private function _csv_data($res){
		...
		while($row = $res->fetch(PDO::FETCH_ASSOC)){
			foreach($row as $fld=>$v){
				if(in_array($fld,array('type','history_id','note2','note3'))){
					continue;
				}else{
					$data[$fld] = $v;
				}
