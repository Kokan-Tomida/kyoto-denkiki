...
class inquiry_guest_confirm extends Base{
	...
	private function _makeForm(){
		...
		if(axts('inquiry_check',$this->m_value)){
			$k = axts('inquiry_check',$this->m_value);
			//if((count($this->m_value['inquiry_check']) > 0) and (is_array($this->m_value['inquiry_check']))){
				...
				//}
				$this->m_value['inquiry_check_view'] .= '</ul>';
				$this->m_value['inquiry_check_save'] = $INQUIRY_CHECKBOX[$k];
	...
	private function _to_regist(){
		...
		$inquiry_check_strings = '';
		$val = $this->m_value['inquiry_check'];
		if((axts($val,$INQUIRY_CHECKBOX)) and ($val == '1')){
			$inquiry_check_strings = '【見積希望】';
		}elseif((axts($val,$INQUIRY_CHECKBOX)) and ($val == '2')){
			$inquiry_check_strings = '【購入検討】';
		}elseif((axts($val,$INQUIRY_CHECKBOX)) and ($val == '3')){
			$inquiry_check_strings = '【資料請求】';
		}elseif((axts($val,$INQUIRY_CHECKBOX)) and ($val == '4')){
			$inquiry_check_strings = '【お問合せ】';
		}
...
*/
		...
		$to = axts('email_address',$this->m_value);
		$data = $this->_makeMailData($datas);
		$to_admin = $this->setFromAddress(MAIL_TYPE8,$pref);
		list($subject,$body) = $this->createMailBodys(MAIL_TYPE8,$data);
		//$body_admin = $this->makeMailDataMemberInfo();
		$body_admin = '';
		$subject_admin = $this->createMailSubject(MAIL_TYPE8,$pref,'',$inquiry_check_strings,'',axts('company_name',$this->m_value));
