...
$temp_html = array(
	'config'		=> 'contact_config.php' // 基本設定
,	'form'			=> 'contact_form.html' // 入力フォーム用
,	'confirm'		=> 'contact_confirm.html' // 確認画面用
,	'completion'	=> 'contact_completion.html' // 送信完了画面
,	'mail'			=> 'contact_mail_tmpl.php' // メール送信用
,	'reply'			=> 'contact_reply_tmpl.php' // 自動返信メール用
);

// エラー表示設定
$temp_err = array(
	'__Error_Input_Data__'		=> '入力必須項目です。'
,	'__Error_Marge_Data__'		=> '入力されていない項目があります。'
...
$temp_err_result = array(
	'form'		=> '<div><span class="fs_crimson">__Result__</span></div>' // フォーム画面用
,	'confirm'	=> '<span class="fs_crimson">__Result__</span>' // 確認画面用
);
...
function printSubmit($error)
{
	...
	if ($error == 1) {
		// エラーがある場合のHTML出力
		$submit = <<< EOD
<div class="pd-left303 pd-top42"><input type="button" name="back" id="back" value="{$button_text['back']}" onclick="history.back()" class="back_btn" /></div>
EOD;
	} else {
		// 項目が正しい場合のHTML出力
		$submit = <<< EOD
<div class="clearfix mg-top58 mg-left232">
<div class="left"><input type="hidden" name="mode" id="mode" value="SEND" /><input type="submit" name="submit" id="submit" value="{$button_text['send']}" class="submit_btn" /></div>
<div class="left"><input type="button" name="back" id="back" value="{$button_text['back']}" onclick="history.back()" class="back_btn" /></div>
</div>
EOD;
	}
	return $submit;
...
class SbFormMailClass
{
	...
	function SbFormMailClass($cfg)
	{
		...
		$WEB_SERVER_NAME = isset($_SERVER['HTTP_X_FORWARDED_SERVER']) ?
		$_SERVER['HTTP_X_FORWARDED_SERVER'] : $WEB_SERVER_NAME;
