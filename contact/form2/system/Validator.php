...
class Validator {
    ...
    const MESSAGE_NOT_EMPTY = '%%%NAME%%%を入力してください';
    const MESSAGE_MUST_SELECT = '%%%NAME%%%を選択してください';
    ...
    const MESSAGE_AGREE = '当フォームのご利用には、同意頂くことが必要となります。';
    ...
    */
    public function check($param) {
        ...
        if ( $checkList['MustSelect'] ) {
            if ( empty($data) ) {
                $message = $this->getMessage($itemName, self::MESSAGE_MUST_SELECT);
                return $message;
            }
        }
        if ( $checkList['Agree'] ) {
            if ( empty($data) ) {
                $message = $this->getMessage($itemName, self::MESSAGE_AGREE);
                return $message;
            }
        }
        ...
        if ( $checkList['Date'] ) {
            // yyyymmddを作成
            $input = $_SESSION[ $sessionName ];

            //  ***がなくて、***_y、***_m、***_dがあるとき用のチェック
            $tmps = array($input[ $param['formName'] . '_y']
                , $input[ $param['formName'] . '_m']
                , $input[ $param['formName'] . '_d']);
            if (implode("", $tmps) != "") {
                $data = $tmps;
        ...
        if ( $checkList['ReturnLaterThan'] ) {
            ...
            //  ***がなくて、***_y、***_m、***_dがあるとき用のチェック
            $data = array($input[ $param['formName'] . '_y']
                , $input[ $param['formName'] . '_m']
                , $input[ $param['formName'] . '_d']);
            $targ = array($input[ $checkList['ReturnLaterThan'] . '_y']
                , $input[ $checkList['ReturnLaterThan'] . '_m']
                , $input[ $checkList['ReturnLaterThan'] . '_d']);
