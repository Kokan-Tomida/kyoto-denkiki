...
class Controller {
    ...
    public function __construct() {
        ...
        $this->sessionStartParamName = 'rent_start_param';
        $this->sessionInputName = 'rent_input';
        $this->sessionErrorName = 'rent_error';
    }
    ...
    public function inputAction() {
        ...
        if ($_GET['back']) {
            ...
            if ($productData) {
                ...
                //$input['check_content'] = $productStr;
            }
            $data['input'] = $input;
        }
    ...
    public function confirmAction() {
        // 入力値をセッションにセット
        $input = $_POST;
        $input = $this->getFormatInputOnConfirm($input);
    ...
    public function getFormatInputOnConfirm($input) {
        ...
        if ($input['extend_cable'] == "2") {
            // 延長ケーブル不要の時、本数を消去
            $input['extend_cable_num'] = "";
        }
        if (is_array($input['extend_length'])) {
            if ( !in_array("9", $input['extend_length']) ) {
                // その他が選択されていない時、その他mを消去
                $input['extend_length_other'] = "";
            }
        }

        /** 表示名取得 **/
        $input['pref_name'] = getPrefName($input['pref']);
        $input['power_need_name'] = getTypeName('power_need', $input['power_need']);
        $input['extend_cable_name'] = getTypeName('extend_cable', $input['extend_cable']);
        if ($input['extend_cable_num']) {
            $input['extend_cable_num_name'] = $input['extend_cable_num'] . '本';
        }
        if ($input['extend_length']) {
            $input['extend_length_name'] = "";
            foreach($input['extend_length'] as $inputLen) {
                if ($input['extend_length_name'] != "") {
                    $input['extend_length_name'] .= ", ";
                }
                $input['extend_length_name'] .= getTypeName('extend_length', $inputLen);
            }
        }
        if ($input['extend_length_other']) {
            $input['extend_length_other_name'] = $input['extend_length_other'] . 'm';
        }
        // 日付フォーマット
        if ($input['rent_y'] && $input['rent_m'] && $input['rent_d']) {
            $input['rent_name'] = date("Y年m月d日", strtotime($input['rent_y'].$input['rent_m'].$input['rent_d']));
        }
        if ($input['return_y'] && $input['return_m'] && $input['return_d']) {
            $input['return_name'] = date("Y年m月d日", strtotime($input['return_y'].$input['return_m'].$input['return_d']));
        }
    ...
    public function _sendCustomerMail($input, $config) {
        ...
        $title = "【貸出】{$input['company_name']}様({$input['pref_name']})";
    ...
    public function validate( $input ) {
        ...
        // 追加
        if ($input['extend_cable'] == "1" && $input['extend_cable_num'] == "") {
            $error["extend_cable_num"] = "延長ケーブル：必要を選んだ場合は、本数をご入力下さい。";
        }
        if (is_array($input['extend_length'])) {
            if (in_array("9", $input["extend_length"]) && $input["extend_length_other"] == "") {
                $error["extend_length_other"] = "ケーブル長：その他を選んだ場合は、長さをご入力下さい。";
            }
        }
